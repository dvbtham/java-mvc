package common;

public class constants {
	public static final String SAVE_ALBUM = "SAVE_ALBUM";  
	public static final String CANCEL_ALBUM = "CANCEL_ALBUM";  
	
	public static final String SAVE_CASI = "SAVE_CASI";  
	public static final String CANCEL_CASI = "CANCEL_CASI"; 
	
	public static final String SAVE_BAIHAT = "SAVE_BAIHAT";  
	public static final String CANCEL_BAIHAT = "CANCEL_BAIHAT"; 
	
	public static final String HOME_ADD_BAIHAT = "HOME_ADD_BAIHAT";  
	public static final String HOME_DELETE_BAIHAT = "HOME_DELETE_BAIHAT"; 
	
	public static final String HOME_ADD_ALBUM = "HOME_ADD_ALBUM";  
	public static final String HOME_DELETE_ALBUM = "HOME_DELETE_ALBUM"; 
	
	public static final String HOME_ADD_CASI = "HOME_ADD_CASI";  
	public static final String HOME_DELETE_CASI = "HOME_DELETE_CASI"; 
	
	public static final String ALBUM_TABLE = "ALBUM_TABLE"; 
	public static final String BAIHAT_TABLE = "BAIHAT_TABLE"; 
	public static final String CASI_TABLE = "CASI_TABLE"; 
	public static final String MUST_PICK = "-- Click để chọn --"; 
}
